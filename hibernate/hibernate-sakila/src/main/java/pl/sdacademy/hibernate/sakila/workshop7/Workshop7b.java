...
public class Workshop7b {
    ...
    public static List<Actor> findActorsByFilmDescription(String descriptionPart) {
        ...
        try {
            final TypedQuery<Actor> filmsQuery = em.createQuery("SELECT DISTINCT a FROM Actor a JOIN FETCH a.films f WHERE" +
                    " f.description LIKE concat('%', :descriptionPart, '%') ORDER BY a.lastName, a.firstName, f.title", Actor.class);
